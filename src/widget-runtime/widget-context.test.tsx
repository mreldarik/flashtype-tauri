...
describe("useWidgetContext", () => {
	test("tracks badge counts via setTabBadgeCount", () => {
		...
		const { result, rerender } = renderHook(useWidgetContext, {
	...
	test("derives base context from parent", () => {
		const parent: WidgetContext = createParentContext({
			isPanelFocused: false,
		});
		const panel: PanelState = {
		...
		const { result } = renderHook(useWidgetContext, {
			initialProps: { panel, isFocused: true, parentContext: parent },
		});
